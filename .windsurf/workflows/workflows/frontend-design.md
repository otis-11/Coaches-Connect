---
description: Create distinctive, production-grade frontend interfaces with high design quality. Use when building web components, pages, dashboards, or when styling/beautifying any web UI.
---
...
Present your design direction to the user for approval BEFORE writing code.
...
## Project-Specific Notes (AI Receptionist)

When applying this workflow to the AI Receptionist project:
- Frontend files live in `frontend/` (Vercel production) and must be mirrored to `src/public/` (Express local dev)
- Pages: `index.html` (login), `dashboard.html`, `admin.html`, `widget.html`
- Current stack: vanilla HTML/CSS/JS, no framework, Google Fonts, inline styles
- CSP header in `frontend/vercel.json` must allow any new font/CDN sources
- Run `/blast-radius` before making changes to understand file sync requirements
